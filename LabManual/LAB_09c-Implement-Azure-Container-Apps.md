...
1. **[コンテナーアプリ]** の画面で、 **[作成]** - **[コンテナーアプリ]** を選択します。

1. 以下の情報を使用して **[基本情報]** タブの詳細を埋めます。
    ...
    | コンテナー アプリ名 | `my-app` |
    | 地域  | East US |
    | Container Apps 環境 | my-environment<br />（「新しい環境の作成」リンクをクリックして「Container Apps 環境の作成」画面に遷移した後、環境名に「my-environment」と入力して「作成」をクリックすると、基本情報タブに戻ります。） |

1. **[コンテナー]** タブで、**[クイックスタート イメージの使用]** のチェックボックスるをオンにします。クイックスタート イメージが **[Simple hello world container]** に設定されていることを確認します。
...
1. **[Your container app is running with a Hello World image]** というメッセージが表示されます。
## 要点
