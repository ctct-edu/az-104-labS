...
4. **[マネージド ディスクの作成]** ブレードで、ディスクを次のように構成します。
...
9. ダウンロードしたファイルを確認し、**`template.json`** が保存されていることを確認します。
   ...
   > **注:** Azure portal の環境によっては、エクスポート時に `template.json` のみがダウンロードされる場合があります。このラボでは、`template.json` に定義されているパラメーターの既定値を使用するため、`parameters.json` は使用しません。
...
7. 次のようにカスタム デプロイ設定を完了します。
   ...
   > **注:** このラボでは `parameters.json` を使用しません。`disk_name` にはテンプレート内の `defaultValue` として `az104-disk2` が設定されています。デプロイ画面で値が表示された場合は、そのまま `az104-disk2` であることを確認してください。

8. **[確認と作成]** を選択し、次に **[作成]** を選択します。

9. デプロイが完了したら **[リソースに移動]** を選択し、**`az104-disk2`** が作成されたことを確認します。

10. **[概要]** ブレードで、リソース グループ **`az104-rg3`** を選択します。`az104-disk1` と `az104-disk2` の 2 つのディスクが表示されていることを確認します。

11. **[設定]** セクションで **[デプロイ]** を選択します。

    > **注:** すべてのデプロイの詳細は、リソース グループに記録されています。テンプレートを使用して大規模な操作を行う前に、いくつかのテンプレート ベースのデプロイが正常に完了することを確認することをお勧めします。

12. リスト内のいずれかのデプロイ名を選択し、**[入力]** と **[テンプレート]** の内容を確認します。
...
1. Azure portal で、右上の **[Cloud Shell]** アイコンを選択します。または、`https://shell.azure.com` にアクセスします。
