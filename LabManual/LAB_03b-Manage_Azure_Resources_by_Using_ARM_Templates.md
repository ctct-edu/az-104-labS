...
![タスク アーキテクチャの図。](./media/az104-lab03-architecture.png)
...
3. **[ディスク]** ブレードの **[リソース]** タブで **[+ 作成]**　を選択します。
...
9. 通知（画面上部のベル状アイコン）を監視し、デプロイ後に **[リソースに移動]** を選択します。

10. **[オートメーション]** セクションから、**[テンプレートのエクスポート]** を選択します。

11. **[ダウンロード(Download)]** を選択し、テンプレートをローカル ドライブに保存します。

12. ダウンロードしたファイルを確認し、**`template.json`** が保存されていることを確認します。
...
5. **[ファイルの管理]** アイコンを使用して、**ファイルのアップロード** でタスク 1 でダウンロードした **`template.json`** を Cloud Shell にアップロードします。
...
7. **[エディター]**（ペン状のアイコン）を選択し、`template.json` を開きます。
...
1. 引き続き **[Cloud Shell]** で **[切り替え先 Bash]** を選択してBashでのコマンド操作に切り替えます。
...
3. 先ほどと同様に、**[エディター]**（ペン状のアイコン）を選択し、`template.json` を開きます。
...
2. ラボで使用するファイル群から **`Allfiles/Labs/03/azuredeploydisk.bicep`** ファイルを確認します。
...
3. Cloud Shell に確認した Bicep ファイルを **アップロード** します。

4. **[エディター]**（ペン状のアイコン）を選択し、そのファイルを開きます。
