...
1. **[ストレージ アカウントを作成する]** ブレードの **[基本情報]** タブで、次の設定を指定します (それ以外は既定値のままにします)。

| 設定 | 値 |
| --- | --- |
| サブスクリプション | **AZ-104T00A CSR 2** |
| リソース グループ | **az104-rg7** から始まる RG |
| ストレージ アカウント名 | 英小文字と数字で構成される、3～24文字のグローバルに一意の名前 |
| リージョン | **(US) East US** |
...
1. **[ネットワーク]** タブで、使用可能なオプションを確認し、パブリックネットワークアクセスの項目で **[無効にする]** を選びます。
...
1. **[セキュリティとネットワーク]** セクションで、**[ネットワーク]** を選択します。 パブリック ネットワーク アクセスが無効になっています。
    + **[パブリック ネットワーク アクセス(Public network access)]** の項目で **[Manage]** をクリックし、それぞれ **[有効(enable)]** 、 **[選択した仮想ネットワークと IP アドレスから有効(Enable from selected networks)]** に変更します。
    + **[IPv4 Addresses]** セクションで、**[クライアント IP アドレスの追加(Add your client IPv4 address)]** をクリックします。
    + 変更を **保存(save)** します。 
...
1. **[データ管理]** セクションで、**[ライフサイクル管理]** を選び、**[規則の追加]** を選びます。
    ...
    + **[基本 BLOB]** タブで、基本 BLOB が最後に変更された時期が ` 30 ` 日以上前の "場合は"、**クール ストレージに移動します**。 
...
1. **[+ コンテナーの追加]** をクリックし、次の設定のコンテナーを**作成**します。
...
1. 作成したストレージ アカウントを開きます。

1. 左側のメニューを下にスクロールし、**[データ ストレージ]** を展開します。

1. **[クラシック ファイル共有]** をクリックします。

1. **[+ クラシック ファイル共有]** をクリックし、**[基本情報]** タブでファイル共有に `share1` という名前を付けます。 
...
1.  作成したストレージ アカウントに戻り、 **[ストレージ ブラウザー]** を選びます。 Azure ストレージ ブラウザーは、アカウントのすべてのストレージ サービスをすばやく表示できるポータル ツールです。

1. **[クラシック ファイル共有]** を選び、**share1** ディレクトリが存在することを確認します。
...
### (オプション) ストレージ アカウントへのネットワーク アクセスを制限する

この手順では、ストレージ アカウントへのアクセスを特定の仮想ネットワークとサブネットに制限します。
ストレージ アカウント側で仮想ネットワークを追加すると、対象サブネットに Azure Storage のサービス エンドポイントが自動的に構成されます。

1. ポータルで、**[仮想ネットワーク]** を検索して選択します。

2. **[+ 作成]** を選択します。リソース グループを選択し、仮想ネットワークに `vnet1` という名前を付けます。

3. その他の設定は既定値のまま、**[レビューと作成]**、**[作成]** の順に選択します。

4. 仮想ネットワークのデプロイが完了したことを確認します。

5. このラボで使用している **ストレージ アカウント** に戻ります。

6. **[セキュリティとネットワーク]** セクションで、**[ネットワーク]** を選択します。

7. **[リソース設定: 仮想ネットワーク、IP アドレス、例外]** の **[Manage]** を選択します。

8. **[Virtual Networks（仮想ネットワーク）]** セクションで、**[Add existing virtual network (既存の仮想ネットワークを追加する)]** を選択します。
...
10. **[有効化]**を選択し、**[追加]** を選択します。

11. 変更を **[Save（保存）]** します。

    > **注:** Azure portal からストレージ アカウントの仮想ネットワーク規則を追加すると、
    > 対象サブネットに Azure Storage のサービス エンドポイントが自動的に構成されます。

### サービス エンドポイントを確認する

12. ポータルで **[仮想ネットワーク]** を検索して選択し、`vnet1` を開きます。

13. `vnet1` の **[サブネット]** を選択し、`default` サブネットを開きます。

14. サービス エンドポイントの設定を確認し、Azure Storage 用のサービス エンドポイントが構成されていることを確認します。

    > **注:** 環境によって、Azure Storage のサービス エンドポイントは
    > `Microsoft.Storage` または `Microsoft.Storage.Global`
    > として表示される場合があります。

### ネットワーク制限を確認する

15. ストレージ アカウントの **[ネットワーク]** に戻ります。

16. 以前追加したクライアント IP アドレスが登録されている場合は、その IP アドレスを削除します。

17. 変更を **[保存]** します。

    > **注:** この設定では、ストレージ アカウントへのアクセスは、
    > 許可した `vnet1` の `default` サブネットからの通信に制限されます。

18. **[ストレージ ブラウザー]** を選択し、ページを更新します。

19. ファイル共有または BLOB コンテンツにアクセスします。

    > **注:** 現在のブラウザーは `vnet1` 内から接続していないため、
    > 「この操作を行う権限がありません」などのアクセス エラーが表示されることを確認します。
    > ネットワーク設定の反映には数分かかる場合があります。
