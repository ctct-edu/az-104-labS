...
1. **[設定]** セクションで **[サービス エンドポイント]** ブレードを選択します。

    + **[サービス エンドポイントの追加]** を選択します。

    + **[サブネット]** ドロップダウンで **[default - 10.0.0.0/24]** サブネットにチェックを入れます。
...
1. **[リソース設定: 仮想ネットワーク、IPアドレス、例外]** セクションの **[Manage]** をクリックします。

1. **[仮想ネットワークを追加する(Add a virtual network)] - [Add existing virtual network]** を選択し、**vnet1** と**default** のサブネットを選び、**[追加]** を選びます。

1. 追加が完了したら、以前追加したマシンの IP アドレスを**削除**します。 許可されるトラフィックは、仮想ネットワークからの着信のみになるはずです。 

1. 変更を必ず **保存(Save)** してください。
