...
1. [仮想マシン] ブレードで、**[+作成]** → **[仮想マシン]** を選択します。

1. [基本情報] タブ上で、次の情報を使用してフォームに入力し、**[次: ディスク >]** をクリックします。 指定されていない設定項目は、既定値のままにしてください。
...
1. **[ネットワーク]** タブ上の [仮想ネットワーク] で、**[Edit virtual network]** を選択します。
...
1. 次の情報を使用して仮想ネットワークを構成し、**[OK]** を選択します。 必要に応じて、既存の情報を削除または置換します。
    ...
    | サブネット名 | `Core` (ペンマークから編集) |
    | 開始アドレス | `10.0.0.0` |
    | サイズ | `/24` |
   
1. サブネット アドレスの範囲が **`10.0.0.0 ～ 10.0.0.255`** になっていることを確認します。

1. 設定内容を確認し、**[保存]** または **[OK]** を選択します。
...
1. [仮想マシン] ページで、**[作成]** を選択し、**[仮想マシン]** を選択します。
...
1. 次の情報を使用して仮想ネットワークを構成し、**[OK]** を選択します。  必要に応じて、既存のアドレス範囲を削除または置換します。
    ...
    | サブネット名 | `Manufacturing`(ペンマークから編集) |
...
| --------------------------------------------- | ------------------------------------- |
| **リモート仮想ネットワークの概要**                              |                                       |
| ピアリング リンクの名前                         | `ManufacturingVnet-to-CoreServicesVnet` |
| リソース ID を知っている | 未選択 |
| サブスクリプション | **MOC Subscription** |
...
1. **[サブネット]** → **[+ 作成]** を選択します。 以下の表の設定を入力して **追加**してください。 
...
1. Azure portal 内で、「`ルートテーブル`」を検索して選択し、**[+作成]** を選択します。 
    以下の表の設定を入力して、**[確認および作成]** → **[作成]** と進んでルートテーブルをデプロイします。
    ...
    | リソース グループ | `az104-rg5`  |
    | リージョン | **East US** |
    | 名前(Name) | `rt-CoreServices` |
    | ゲートウェイのルートを伝達する(Propagate gateway routes) | **No** |
