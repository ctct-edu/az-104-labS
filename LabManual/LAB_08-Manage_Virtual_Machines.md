...
このタスクでは、Azure Portal を使用して、
2 台の Azure 仮想マシンを異なる可用性ゾーンにデプロイします。

2 台の VM は同じ仮想ネットワーク、同じサブネットに配置し、
可用性ゾーンだけを分けます。

> **注:** サブネットは論理的なネットワーク範囲です。  
> 同じサブネット内の VM でも、異なる可用性ゾーンに配置できます。

同じ Azure リージョン内の複数の可用性ゾーンに VM を分散することで、
データセンター単位の障害に対する可用性を高めることができます。

1. Azure Portal にサインインします。

1. **[仮想マシン]** を検索して選択し、**[作成]** → **[Azure 仮想マシン]** を選択します。

1. まず、1 台目の仮想マシンを次の設定で作成します。

   | 設定 | 値 |
   | --- | --- |
   | サブスクリプション | 既定のサブスクリプション |
   | リソース グループ | **az104-rg8** から始まるリソース グループ |
   | 仮想マシン名 | `az104-vm1` |
   | リージョン | **(US) West US 2** |
   | 可用性オプション | **可用性ゾーン** |
   | 可用性ゾーン | **ゾーン 1** |
   | セキュリティの種類 | **Standard（標準）** |
...
1. 仮想ネットワークとサブネットは、画面に表示される既定値を使用します。  
   ここで使用している **仮想ネットワーク名とサブネット名を確認しておきます**。

   > **注:** 2 台目の VM も、ここで使用したものと同じ仮想ネットワーク、同じサブネットに配置します。

1. ロード バランサーは構成せず、その他のネットワーク設定は既定値のままとします。

1. **[次へ: 管理]** を選択します。

   パッチ オーケストレーション オプションは、
   既定の **[OS による自動処理 (Windows の自動更新)]** のままとします。

1. **[次へ: 監視]** を選択し、**[ブート診断]** を **[無効化]** にします。

1. **[確認および作成]** → **[作成]** を選択します。

1. `az104-vm1` の作成が完了したら、同じ手順でもう 1 台 VM を作成します。
...
1. **[ネットワーク]** タブでは、`az104-vm1` で使用したものと同じ **仮想ネットワーク** と **サブネット** を選択します。

1. その他の設定は `az104-vm1` と同じにして、`az104-vm2` を作成します。

> **確認ポイント:**  
> `az104-vm1` と `az104-vm2` は同じ VNet / Subnet に所属していますが、  
> `az104-vm1` は Zone 1、`az104-vm2` は Zone 2 に配置されます。
...
1. ディスクが作成されたら、**[デタッチ]** (プラグを抜くようなアイコン)をクリックし、**[適用]** を選択してください。
    ...
    > 後続の手順で操作に失敗する場合は、まだデタッチ処理が完了していない可能性があります。その場合は、しばらく待ってから画面を更新し、再度操作してください。

1. **[ディスク]** を検索して選択します。

1. **[Azure ディスク]** の画面で、上部の **[リソース]** タブをクリックします。

1. ディスクの一覧から **vm1-disk1** を選択します。

1. **[設定]** セクションから **[サイズおよびパフォーマンス]** を選択してください。

1. ストレージの種類を **Standard SSD** に設定し、**[保存]** を選択してください。

1. 次の警告が表示されて保存できない場合は、ディスクがまだ仮想マシンにアタッチされている、またはデタッチの反映が完了していない可能性があります。表示されない場合は、項番14から引き続き取り組んでください。

   > ディスクのサイズを変更できるのは、ディスクがアタッチされていない場合か、管理用の仮想マシンが割り当て解除されている場合のみです。

1. その場合は、対象の仮想マシンに戻り、**タスク 2 の項番 6 で実施したディスクのデタッチ操作が完了していること**を確認します。まだディスクが接続されている場合は、もう一度 **[デタッチ]** を実行して保存します。

   > **注:** **[デタッチ]** のアイコンが表示されていない場合は、デタッチ処理中の可能性があります。  
   > 少し待ってから **[最新の情報に更新]** を実行し、ディスクが一覧から消えたことを確認してください。

1. デタッチが完了したら、再度ディスクの **[サイズおよびパフォーマンス]** を開き、ストレージの種類を **Standard SSD** に変更して **[保存]** を選択します。
...
1. **[規則を追加する]** を選択し、設定を調整してから、**[追加]** を選択してください。
...
1. **vmss1** ページで、左ペインから **[インスタンス]** を選択してください。 ここでは、仮想マシン インスタンスの数を監視します。
