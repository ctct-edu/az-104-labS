...
このタスクでは、Azure portal を使用して、2 台の Azure 仮想マシンを異なる可用性ゾーンにデプロイします。

同じ Azure リージョン内の 2 つ以上の可用性ゾーンに 2 台以上の VM を配置することで、
少なくとも 1 台の VM への接続について 99.99% の SLA が適用されます。

1. Azure portal にサインインします。

2. [仮想マシン] を検索して選択し、[作成] > [Azure 仮想マシン] を選択します。

3. まず、1 台目の仮想マシンを次の設定で作成します。
...
5. [次へ: ネットワーク] を選択し、既定値を使用します。
   ロード バランサーは構成しません。

6. [次へ: 管理] を選択します。

   パッチ オーケストレーション オプションは、
   既定の [OS による自動処理 (Windows の自動更新)] のままにします。

7. [次へ: 監視] を選択し、[ブート診断] を [無効化] にします。

8. [確認および作成] > [作成] を選択します。

9. `az104-vm1` の作成が完了したら、同じ手順でもう 1 台 VM を作成します。
...
その他の設定は `az104-vm1` と同じにします。

10. デプロイが完了するまで待ってから、[リソースに移動] を選択してください。
