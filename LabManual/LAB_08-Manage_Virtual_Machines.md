...
1. **「次: Monitoring >」** で、**「仮想マシンの作成」** ブレードの **「Monitoring」** タブでは設定変更をせずに内容のみを確認し、**「次: 詳細>」** をクリックします。
...
1. **「仮想マシンスケールセットの作成」** ブレードの **「ネットワーク」** タブに戻り、**「負荷分散」** セクションで、**「ロード バランサーを使用する」** エントリが選択されていることを確認し、次の**負荷分散設定**を指定して (他の設定はデフォルト値のままにします)、**「次: スケーリング >」** をクリックします。
    ...
    | ロード バランサーを選択します | **(新規) az10408vmss0-lb** |
    | バックエンド プールの選択 | **(新規) bepool** |
