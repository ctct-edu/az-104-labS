...
1. ポータルで、「**監視**」を検索して選びます。

1. **[VM Insights]** ボックスで **[表示]** を選んでから、**[分析情報の構成]** を選びます。

1. お使いの仮想マシンが「監視対象」としてカウントされていたら、以下の手順を割愛して、タスク２にジャンプしてください。そうでなかれば以下の手順を実施します。

1. お使いの仮想マシンの右にある **[有効にする]** ボタンを選択します。

1. 「[クラシック ] ログベースのメトリック」チェックボックスをオンにします。

1. **[レビューと有効化]** -  **[有効にする]** を選択します。

1. 環境をオンボードするには数分かかります。その後、次のステップに進みます。 
...
1. 検索欄に **Microsoft.Compute/virtualMachines/delete** と入力し、**[仮想マシンの削除 (仮想マシン)]** を見つけて選びます。 **[適用(Apply)]** を選択します

1. **[アラート ロジック]** 領域 (下にスクロール) で、**[イベントのレベル]** の選択を確認します。 既定値の **[すべて選択済み]** のままにします。
...
1. **[次へ: 通知]** を選び、「通知」タブに進みます。

1. １行目の「通知 タイプ」欄のドロップダウンリストで  **[電子メール/SMS メッセージ/プッシュ/音声]** を選択します。

1. 「電子メール/SMS メッセージ/プッシュ/音声」の画面が表示されたら、**[電子メール]** を選択し、**[電子メール]** ボックスに、もし可能であれば任意の受信可能なメール アドレスを入力して、**[OK]** を選択します。**[確認と作成]**  、 **[作成]** を選択します。

1. 「通知」タブに戻ったら、１行目の「名前」欄に  `VM was deleted`　と記入します。

1. **[確認と作成]** を選択して入力を検証してから、**[作成]** を選択します。

    >**注:**  アクション グループに追加されたことを示すメール通知が届くはずです。 数分間何も起こらないことがあり得ますが、それはルールがデプロイされていることの確実な証拠です。

1. アクション グループが作成され、前のタスクの「アラート ルールの作成」ウィザードに戻ってきました。
    **[次へ: 詳細]** タブに移動します。
...
1. **vm0** の削除により、3 つの詳細アラートが生成されます。

   >**注:**  ポータルでアラートが更新されるまで、数分かかる場合があります。 待ちたくない場合は、次のタスクに進んでから、戻ってください。また、テストのため仮想マシンを２回以上削除したい場合、タスク1のjsonファイルによるプロジョニングを再実行すれば、仮想マシンは再展開されます。

   いずれかのアラートの名前を選択します (例: **[VM was deleted] (VM が削除されました)**)。 イベントの詳細を示す **[アラートの詳細]** ペインが表示されます。
...
1. 自分の**リソース グループ（az104-rg11）**を選んで、**[適用]** を選びます。
   
1. **[次へ: ルールの設定]** を選択し、**[通知を表示しない]** を選択します。
...
警告処理ルールのスケジュール設定に、次の設定を入力してください。
   ...
   |---------|---------|
   | ルールの適用 | 特定時刻 |
   | 開始 | 今日の日付の午後 10 時を入力します。 |
   | 終了 | 明日の日付の午前 7 時を入力します。 |
...
1. Azure portal で `監視` ブレードを見つけて選び、**[ログ]** をクリックします。

1. 必要な場合はスプラッシュ画面を閉じます。 

1. スコープとして自分の**リソース グループ**を選びます。 **適用**を選択します。

1. 画面右上の **[クエリ ハブ]** のボタンを選択します。

    (ボタンが見つからない場合は、一度左のナビゲーションメニューで [ログ] 以外のメニューをクリックした後、再び [ログ]  をクリックして戻ると、既定では [クエリハブ]が表示されます)

1. **[クエリ ハブ]** で、(左側のペインで) **[Virtual machines]**  を選びます。 

1. 使用できるメトリックを確認します。 **[Count heartbeats] (ハートビート数)** クエリを **[実行]** します。

1. 仮想マシンが実行していたときのハートビート数が表示されます。

1. 結果のひだりうえにある **[ユーザー ク...]** を選択してクエリを確認します。以下のようなクエリとなっている可能性が高いです。  
    ...
    このクエリでは、*heartbeat* テーブルが使われています。 
