...
+ タスク 5:仮想マシンのレプリケーションを有効にします。 （オプション）
...
1. **\\Allfiles\\Lab\\10\\az104-10-vms-edge-template.json** ファイルを見つけて選択し、**[開く]** を選択します。
...
1. **\\Allfiles\\Lab\\10\\az104-10-vms-edge-parameters.json** ファイルを読み込んで選択します。
...
1. **[確認および作成]** をクリックし、検証が成功したことを確認してから **[作成]** をクリックします。
...
1. [az104-rsv-region1 | プロパティ] ブレードに戻り、画面を下の方にスクロールして「セキュリティ設定」をみつけます。そのさらに下の方にある **[論理的な削除の設定]** ラベルの下の **[更新]** リンクをクリックします。

1. **[論理的な削除の設定]** ブレードで、  **[論理的な削除の保持期間]** が **14** 日であることを確認してください。 
...
1. **[仮想マシンの選択]** ブレードで、**[az-104-10-vm0]** を選択して **[OK]** をクリックしてから、**[バックアップ]** ブレードに戻り、**[バックアップの有効化]** をクリックします。
    ...
    >**注**:バックアップが有効になるまで待ちます。 これには約 2 分かかります。「ConfigureProtection-XXXXXXXXXXXXX」ブレードで「デプロイが完了しました」と表示されたら「リソースに移動」をクリックして、az104-rsv-region1(Recovery Services コンテナー)のブレードに戻ります。
...
## タスク 4:Azure Backup を監視する（オプション）
...
(事前準備) ※演習環境で Microsoft.insights を登録することで、手順10前後の「診断設定」を可能にします。

Azure Portalの上のボタンから「Azure Cloud Shell」を起動します。
「Azure Cloud Shell へようこそ」画面で「Bash」を選択します。
...
1. Azure portal から、`ストレージアカウント` を検索して選択します。

1. [ストレージ センター | Blob Storage] ブレードで、**[作成]** を選択します。

1. 次の情報を使用してストレージ アカウントを定義してから、**[レビューと作成]** を選択します。
...
1. **[監視]**セクションの**[診断設定]**、**[診断設定を追加する]** の順に選択します。
...
## タスク 5:仮想マシンのレプリケーションを有効にする（オプション）
...
1. **[Recovery Services コンテナーの作成]** ウィンドウで、次の設定を指定します。
    ...
    | --- | --- |
    | サブスクリプション | 既定のサブスクリプション |
    | リソース グループ | `az104-rg-region2` (新規作成) |
...
1. **[基本]** タブで、**[ターゲット リージョン]** が、**2つ目の Recovery Services コンテナーがあるリージョンであることを確認** してください。

1. **[次へ：詳細設定]** を選択して、次のタブに移動します。リソースの選択が自動的に行われました。 それらを確認することが重要です。 

1. **[拡張機能の設定]** で、Automation アカウントの **[作成]** を選択します。作成が完了するまで1分ほど待ちます。

1. **[レプリケーションを確認して開始する]**、**[レプリケーションの開始]** の順に選択します。
...
1. **[保護されたアイテム]** セクションで、**[レプリケートされたアイテム]** を選択します。
...
>今回はここまでとなりますが、フェールオーバーを試す場合の流れは以下をご参照ください。
>
>[保護された VM のフェールオーバーをテストする](https://learn.microsoft.com/azure/site-recovery/tutorial-dr-drill-azure#run-a-test-failover-for-a-single-vm)
