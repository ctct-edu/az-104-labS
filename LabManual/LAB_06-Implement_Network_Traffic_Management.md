...
1. Azure portal で「`ロードバランサー`」を検索して選び、**[ロード バランサー]** ブレードで **[+ 作成]** をクリックして、Standard Load Balancerを選択します。
...
1. 別のブラウザー タブを開き、その IP アドレスに移動します。 ブラウザー ウィンドウに、"**Hello World from az104-06-vm0**" または "**Hello World from az104-06-vm1**" というメッセージが表示されることを確認します。
    ...
    ※ `ブラウザーが自動でhttps://～にアクセスさせることがあります。今回はhttp://～でアクセスする必要がありますので、もしアクセスできない場合はアクセスするURLが http://～ となっているか確認してください。`
...
1. Azure portal で「`Application gateway(アプリケーションゲートウェイ)`」を検索して選び、**[アプリケーション ゲートウェイ]** ブレードで **[+ Create(作成)]** をクリックしてドロップダウンリストからApplication Gatewayを選択します。
...
1. **[パスベースの規則を作成するには複数のターゲットを追加します]** を選びます。 2 つの規則を作ります。 それぞれの設定を入力して **[追加]** をクリックします。 
