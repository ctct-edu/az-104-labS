...
このタスクでは、Azure portal を介してタグを作成し、Azure リソース グループに割り当てます。 タグはガバナンス戦略の重要なコンポーネントです。 タグを使用すると、リソース所有者、期限日、グループ連絡先、さらに組織が重要と見なすその他の名前と値のペアをすばやく識別できます。 このタスクでは、リソースのロールを識別するタグ ("インフラストラクチャ" の場合は "Infra") を割り当てます。

1. [**Azure portal**](https://portal.azure.com)にサインインします。
   
1.  **[リソースグループ]** を検索して選択します。

1. [リソース グループ] から **[+ 作成]** を選択して、新規にリソースグループを作成します。
...
1. **[作成]** セクションから **[定義]** を選択します。使用できる[組み込みポリシー定義](https://learn.microsoft.com/azure/governance/policy/samples/built-in-policies)のリストが確認できます。

1. **[Require a tag and its value on resources] (タグとその値がリソースに必要)** という組み込みポリシーを表すエントリを検索してクリックします。

1. **[タグとその値がリソースに必要]** ポリシーの定義のブレードで、**[ポリシーの割り当て]** をクリックします。

1. **スコープ** 欄の右にある省略記号ボタンをクリックして次の値を指定します。 完了したら、**[選択(Select)]** をクリックします。 
...
1. 次の設定を指定して、割り当ての **基本情報**プロパティを構成します (その他は既定値のままにします)。
...
1. **[レビューと作成]** をクリックしてから、**[作成]** をクリックします。
...
1. ポータルで、**[ストレージ アカウント]** を検索して選択し、**[+ 作成]** を選択します。

1. **[ストレージ アカウントの作成]** ブレードの **[基本情報]** タブで、次の構成を入力します。

   | 設定 | 値 |
   | --- | --- |
   | リソース グループ | **az104-rg2** |
   | ストレージ アカウント名 | 任意の名称を指定<br />※グローバルに一意であり、3～24文字の英小文字と数字の組み合わせにする必要があります。 |
   | プライマリ サービス | **Azure Blob Storage または Azure Data Lake Storage** |

1. **[プライマリ サービス]** で、**[Azure Blob Storage または Azure Data Lake Storage]** を選択します。

1. その他の設定は既定値のままとし、**[レビューと作成]** をクリックします。

1. 作成可能かどうかの検証が行われるため、検証が完了するまで待ちます。

1. **"検証失敗"** のメッセージが表示されるはずです。 メッセージを確認して失敗の原因を特定します。 リソースのデプロイがポリシーによって許可されなかったことをエラー メッセージが示しているのを確認します。  
...
1. 割り当ての一覧で、**[タグとその値がリソースに必要]** ポリシーの行の右端にある **[…]（3点リーダー）** をクリックし、**[割り当ての削除]** を選択して、このポリシーの割り当てを削除します。

1. **[ポリシーの割り当て]** をクリックし、先ほどと同様に省略記号ボタンをクリックして次の値を選択し、**[スコープ]** を指定します。
...
1. **ポリシー定義**を指定するには、省略記号ボタンをクリックし、`Inherit a tag from the resource group if missing`（存在しない場合は、リソース グループからタグを継承する） を検索して選択します。

1. **[追加]** を選択し、割り当ての残りの **[基本情報]** プロパティを構成します。

    | 設定 | 値 |
    | --- | --- |
    | 割り当て名 | 存在しない場合は、リソース グループからタグを継承する |
    | 説明 | 存在しない場合は、リソース グループからタグを継承する |
    | ポリシーの適用 | 既定値 |
...
1. **[次へ]** をクリックし、**[修復]** タブで次の設定を構成します (その他の設定は既定値のままにします)。
    ...
    | 設定 | 値 |
    | --- | --- |
    | 修復タスクを作成する | 有効（チェックを入れる） |
    | 修復するポリシー | **Inherit a tag from the resource group if missing（存在しない場合は、リソース グループからタグを継承する）** |
...
1. **[ストレージ アカウントの作成]** ブレードの **[基本情報]** タブで、ポリシーが適用されたリソース グループを使用していることを確認し、次の設定を指定します。その他の設定は既定値のままにします。
...
1. 設定が完了したら、**[レビューと作成]** をクリックします。

1. 今度は検証に成功したことを確認し、**[作成]** をクリックします。

1. 新しいストレージ アカウントのプロビジョニングが完了したら、**[リソースに移動]** をクリックします。

1. **[タグ]** ブレードで、値が **000** のタグ **Cost Center** がリソースに自動的に割り当てられていることを確認します。
...
1. リソース グループの一覧から、作成した **az104-rg2** をクリックします。
...
1. リソースグループの **[概要]** ブレードに移動し、**[リソースグループの削除]** を選択します。

1. **[削除を確認するために、リソース グループ名を入力してください]** テキストボックスに、リソース グループ名「`az104-rg2`」を入力します。 リソース グループ名はコピーして貼り付けることができます。 

1. **削除**を選択します。

    > 警告を確認してください。このリソース グループとその依存リソースの削除は永続的な操作であり、元に戻すことはできません。
