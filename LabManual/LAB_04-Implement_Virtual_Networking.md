...
1. CoreServicesVnet の **[基本情報]** タブに入力します。  
...
1. **[アドレス空間]** タブに移動します。IPv4 アドレス空間をデフォルトの値から変更し、**10.20.0.0/16** に設定します。

1. **[+ サブネットの追加]** を選択し、1つ目のサブネットとして次の内容を入力します。入力後、**[追加]** を選択します。

    | 設定 | 値 |
    | --- | --- |
    | 名前 | `SharedServicesSubnet` |
    | 開始アドレス | `10.20.10.0` |
    | サイズ | `/24` |

1. もう一度 **[+ サブネットの追加]** を選択し、2つ目のサブネットとして次の内容を入力します。入力後、**[追加]** を選択します。
...
1. デフォルトで作成されているサブネットがある場合は、そのサブネットを選択して削除します。

1. 最終的に、次の2つのサブネットだけが残っていることを確認します。
...
1. CoreServicesVnet とサブネットの作成を終えるには、**[レビューと作成]** を選択します。
...
1. テンプレートを**ダウンロード**します。**template.json** ファイルがローカル コンピューターにダウンロードされます。

1. ローカル コンピューター上で、**ダウンロード** フォルダーに移動し、**template.json** ファイルがダウンロードされていることを確認します。

1. **template.json** ファイルを確認します。このテンプレートを使用して、次のタスクで ManufacturingVnet を作成します。
...
1. 任意のエディター（メモ帳など）を使用してファイルを編集します。
...
1. **[レコード セット]** を選択します。 **[+ 追加]** のボタンをクリックして以下の値を設定します。
...
1. **[追加]** をクリックし、**www** という名前の A レコード セットがあることを確認します。

1. Windows上でコマンド プロンプトを開いて、次のコマンドを実行します。<作成したドメイン名>には前の手順で作成したドメイン名を入力します。<name server name>には、以前のタスクで確認したネームサーバーのアドレスを入力します。

    ```sh
    nslookup www.<作成したドメイン名> <name server name>
    ```
...
1. プライベート DNS ゾーンのデプロイを待ってから、**[リソースに移動]** を選択します。

1. **[概要]** ブレードにネーム サーバー レコードがないことに注意してください。 

1. **[DNSの管理]** セクションから **[+ 仮想ネットワーク リンク]** を選択し、**[+ 追加]** を選択します。 
...
1. **[DNSの管理]** セクションから、**[レコード セット]** を選択します。 ここで、プライベート名前解決のサポートが必要な仮想マシンごとにレコードを追加します。
