...
1. **[テナントを作成する]** 画面で、次の項目を入力します。
    ...
    | -------------- | ------------------------------------------------------------ |
    | テナント名         | **First AAD**                                                |
    | ドメイン名         | **firstaadXXXXXXXX**<br />※Xにはユーザー名(LabUser-XXXXXXXX)と同じ8桁の数字を入力します。(あるいはランダムで一意な数字) |
    | 国/地域           | **米国**                                                     |
...
1. 作成した **First AAD** の横に表示されている **[切り替え]** をクリックして、新しく作成したテナントへ切り替えます。
   ...
   > 一覧に作成したテナント（ディレクトリ）が表示されない場合はページを更新します。
...
1. **[メンバーの追加]** ペインで、作成した **az104-user1** と招待した **Lab User** を検索してチェックボックスにチェックを入れて選択します。これでユーザーがグループに追加されました。 
