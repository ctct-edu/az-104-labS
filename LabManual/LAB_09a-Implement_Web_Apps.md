...
1. **「Web アプリ」** ブレードの **「基本」** タブで、次の設定を指定します (他の設定は既定値のままにします)。
    ...
    | リソース グループ | **az104-09a-rg1**から始まるリソースグループを選択 |
    | 名前 | **Skillableのラボウィンドウに記載されている"Web app name"と同じ名称を入力** |
    | 公開 | **コード** |
    | ランタイム スタック | **PHP 8.0** |
    | オペレーティング システム | **Linux** |
    | 地域 | East US |
...
1. **「+ Add Slot」** をクリックし、次の設定をした後 **「Add」「Close」** をクリックし新しいスロットを追加します。
...
1. **「Settings」** タブの **「Source」** ドロップダウン リストで、**「Local Git」** を選択し、**「Save」** ボタンをクリックします

1. **「デプロイ センター」** ブレードで、**「Git Clone URI」** エントリをメモ帳にコピーします。

    >**注:** この Git Clone URI の値は、ラボの次のタスクで必要です。

1. **「デプロイ センター」** ブレードで、**「Local Git/FTPS credentials」** タブを選択し、**「User scope」** セクションで次の設定を指定して、**「Save」** をクリックします。
...
1. ステージング スロットのブレードで **「概要」** 、 **「既定のドメイン」** の下のURLリンクの順にクリックして、新しいブラウザー タブで既定の Web ページを表示します。
...
1. App Serviceの運用スロットを表示するブレードに戻ります。(Azureポータル上の検索欄を App Service で検索した後、表示されたWebアプリ名をクリックする等、いくつか方法があります。)

1. **「デプロイ」** セクションで **「デプロイ スロット」**、**「スワップ」** の順にクリックします。

1. **「スワップ」** ブレードで、設定が既定のままであることを確認してから **「スワップ」** をクリックします。

1. **「概要」** をクリックしてから **「既定のドメイン」** の下のURLリンクをクリックして、新しいブラウザー タブに Web サイトのホーム ページを表示します。
...
1. App Serviceの運用スロットを表示するブレードの **「設定」** セクションで、**「スケールアウト (App Service のプラン)」** をクリックします。
    ...
    > **注:** 日本語にローカライズされていないメニューが表示される場合があります。その場合は **Scaling** の項目から **「Rules Based」** を選択して、**「Manage rules based scaling」** をクリックします。
