...
1. **「+Add slot 」** をクリックし、次の設定で新しいスロットを追加します。
...
1. ステージング スロットで、**[デプロイ]**、**[デプロイ センター]** の順に選びます。

    >**注:**  運用スロットではなく、ステージング スロットのブレードであることを確認します。

1. **[設定]** タブで、「優先するソースおよびビルド プロバイダーからコードをデプロイし、ビルドします。」

    の記載の右下にある **[手動デプロイ(プッシュ)]** を選択します。 

1. **[ソース]** ドロップダウン リストで、**[外部の Git]** を選びます。 
...
## (オプション)タスク 5:Azure Web アプリの自動スケーリングを構成およびテストする
...
1. **[概要]** 画面の  **[HTTP 要求を追加して作成する]** 記載のすぐ下にある **[作成]** を選択します。

1. **[URL ベースのテストを作成する]** 画面の  **[Basics]** タブは何も変更せず **[次へ]** を選択します。

1. **[テスト計画]** で **[+要求の追加]** を選択します。

1. **[URL]** に、実際の**既定のドメイン** URL を貼り付けます。 正しく書式設定され、**https://** で始まっていることを確認します。**[追加]** を選択します。
...
1. **[停止]** を選択して停止要求を開始し、確認ダイアログでもう一度 **[停止]** を選択してテスト実行を完了します。 テストが完了するまで待機する必要はありません。
